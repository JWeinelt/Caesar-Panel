...
export(float, 0, 10, 0.01) var speed := 5.0
...
export(float, 0, 1) var damping_scroll := 0.1
## Softness of damping when "overdragging" with dragging
export(float, 0, 1) var damping_drag := 0.1
## Scrolls to currently focused child element
export var follow_focus_ := true
## Margin of the currently focused element
export(int, 0, 50) var follow_focus_margin := 20
...
export(float, 0, 1) var friction_drag := 0.9
## Hides scrollbar as long as not hovered or interacted with
export(bool) var hide_scrollbar_over_time = false setget _set_hide_scrollbar_over_time
...
var is_scrolling := false setget set_is_scrolling

# Define the setter function
func set_is_scrolling(val):
	is_scrolling = val
	if is_scrolling:
...
func _ready() -> void:
	...
	get_h_scrollbar().connect("scrolling", self, "_on_HScrollBar_scrolling")
	get_v_scrollbar().connect("gui_input", self, "_scrollbar_input")
	get_h_scrollbar().connect("gui_input", self, "_scrollbar_input")
	...
	scrollbar_hide_timer.connect("timeout", self, "_scrollbar_hide_timer_timeout")
...
func _process(delta: float) -> void:
	...
	get_v_scrollbar().set_value(-pos.y)
	get_v_scrollbar().update()
	# Update horizontal scroll bar
	get_h_scrollbar().set_value(-pos.x)
	get_h_scrollbar().update()
	# Update state
	update_state()
...
func _scrollbar_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		...
		or event.button_index == BUTTON_WHEEL_UP\
		or event.button_index == BUTTON_WHEEL_LEFT\
		or event.button_index == BUTTON_WHEEL_RIGHT:
			_gui_input(event)
...
func _gui_input(event: InputEvent) -> void:
	...
	v_scrollbar_dragging = get_v_scrollbar().has_focus() # != pressed => TODO
	h_scrollbar_dragging = get_h_scrollbar().has_focus()
		...
		match event.button_index:
			...
			BUTTON_LEFT:
				if event.pressed:
					...
					friction = 0.0
					drag_start_pos = content_node.rect_position
					init_drag_temp_data()
				else:
...
func _on_focus_changed(control: Control) -> void:
	...
	if focus_top < 0.0:
		scroll_y_to(content_node.rect_position.y - focus_top + follow_focus_margin)
	
	if focus_bottom > self.rect_size.y:
		scroll_y_to(content_node.rect_position.y - focus_bottom + self.rect_size.y - follow_focus_margin)
	
	if focus_left < 0.0:
		scroll_x_to(content_node.rect_position.x - focus_left + follow_focus_margin)
	
	if focus_right > self.rect_size.x:
		scroll_x_to(content_node.rect_position.x - focus_right + self.rect_size.x - follow_focus_margin)
...
func _set_hide_scrollbar_over_time(value) -> void:
	hide_scrollbar_over_time = value
	if value == false:
		if scrollbar_hide_timer != null:
			scrollbar_hide_timer.stop()
		if scrollbar_hide_tween != null:
			scrollbar_hide_tween.kill()
		get_h_scrollbar().modulate = Color.white
		get_v_scrollbar().modulate = Color.white
	else:
	...
	if vertical:
		if not allow_overdragging:
			...
			if is_outside_top_boundary(axis_pos):
				...
				axis_pos = self.rect_size.y - content_node.rect_size.y
				axis_velocity = 0.0

		content_node.rect_position.y = axis_pos 
		pos.y = axis_pos
		...
		if not allow_overdragging:
			...
			if is_outside_left_boundary(axis_pos):
				...
				axis_pos = self.rect_size.x - content_node.rect_size.x
				axis_velocity = 0.0

		content_node.rect_position.x = axis_pos
		pos.x = axis_pos
...
func handle_overdrag(vertical: bool, axis_velocity: float, axis_pos: float) -> Array:
	...
	if vertical:
		var size_y = get_size().y
		if get_h_scrollbar().visible:
			size_y -= get_h_scrollbar().rect_size.y
		dist2 += max(size_y - content_node.rect_size.y, 0)
	else:
		var size_x = get_size().x
		if get_v_scrollbar().visible:
			size_x -= get_v_scrollbar().rect_size.x
		dist2 += max(size_x - content_node.rect_size.x, 0)
	...
	if dist1 > 0:
		...
		if dist1 < just_snap_under and abs(axis_velocity) < just_snap_under:
			...
			result[0] = calculate(vertical, dist1, axis_velocity)
	# Overdrag on bottom or right
		...
		if dist2 > -just_snap_under and abs(axis_velocity) < just_snap_under:
			...
			result[0] = calculate(vertical, dist2, axis_velocity)
	
	return result

func calculate(vertical, dist: float, axis_velocity: float) -> float:
	# Apply bounce force
...
func handle_scrollbar_drag() -> bool:
	if h_scrollbar_dragging:
		velocity.x = 0.0
		pos.x = content_node.rect_position.x
		return true
	
	if v_scrollbar_dragging:
		velocity.y = 0.0
		pos.y = content_node.rect_position.y
		return true
	return false

func calc_dest(delta: float) -> float:
	if delta >= 0.0:
		return delta / (1 + delta * damping * 0.1)
	else:
		return delta

func calc_pos(temp_dist1: float,temp_dist2: float,temp_relative: float) ->float:
	if temp_relative + temp_dist1 > 0.0:
		var delta = min(temp_relative, temp_relative + temp_dist1)
		var dest = calc_dest(delta)
...
func calculate_distance() -> void:
	...
	bottom_distance = content_node.rect_position.y + content_node.rect_size.y - self.rect_size.y
	top_distance = content_node.rect_position.y
	right_distance = content_node.rect_position.x + content_node.rect_size.x - self.rect_size.x
	left_distance = content_node.rect_position.x
	if get_v_scrollbar().visible:
		right_distance += get_v_scrollbar().rect_size.x
	if get_h_scrollbar().visible:
		bottom_distance += get_h_scrollbar().rect_size.y
...
func will_stop_within(vertical : bool, vel : float) -> bool:
	...
	var diff = self.rect_size.y - content_node.rect_size.y if vertical else self.rect_size.x - content_node.rect_size.x
...
func draw_debug() -> void:
	# Overdrag lines
	# Top + Bottom
	draw_line(Vector2(0.0, 0.0), Vector2(0.0, top_distance), debug_gradient.sample(clamp(top_distance / rect_size.y, 0.0, 1.0)), 5.0)
	draw_line(Vector2(0.0, rect_size.y), Vector2(0.0, rect_size.y+bottom_distance), debug_gradient.sample(clamp(-bottom_distance / rect_size.y, 0.0, 1.0)), 5.0)
	# Left + Right
	draw_line(Vector2(0.0, rect_size.y), Vector2(left_distance, rect_size.y), debug_gradient.sample(clamp(left_distance / rect_size.y, 0.0, 1.0)), 5.0)
	draw_line(Vector2(rect_size.x, rect_size.y), Vector2(rect_size.x+right_distance, rect_size.y), debug_gradient.sample(clamp(-right_distance / rect_size.y, 0.0, 1.0)), 5.0)

	# Velocity lines
	var origin := Vector2(5.0, rect_size.y/2)
	draw_line(origin, origin + Vector2(0.0, velocity.y), debug_gradient.sample(clamp(velocity.y*2 / rect_size.y, 0.0, 1.0)), 5.0)
	draw_line(origin, origin + Vector2(0.0, velocity.x), debug_gradient.sample(clamp(velocity.x*2 / rect_size.x, 0.0, 1.0)), 5.0)
...
func scroll_x_to(x_pos: float, duration:float=0.5) -> void:
	...
	velocity.x = 0.0
	x_pos = clamp(x_pos, self.rect_size.x-content_node.rect_size.x, 0.0)
...
func scroll_y_to(y_pos: float, duration:float=0.5) -> void:
	...
	velocity.y = 0.0
	y_pos = clamp(y_pos, self.rect_size.y-content_node.rect_size.y, 0.0)
	var tween = create_tween()
...
func scroll_page_up(duration:float=0.5) -> void:
	var destination = content_node.rect_position.y + self.rect_size.y
...
func scroll_page_left(duration:float=0.5) -> void:
	var destination = content_node.rect_position.x + self.rect_size.x
...
func scroll_to_bottom(duration:float=0.5) -> void:
	scroll_y_to(self.rect_size.y - content_node.rect_size.y, duration)
...
func should_scroll_horizontal() -> bool:
	var disable_scroll = content_node.rect_size.x - self.rect_size.x < 1 or not allow_horizontal_scroll\
...
func hide_scrollbars() -> void:
	if scrollbar_hide_tween != null:
		scrollbar_hide_tween.remove_all()
		scrollbar_hide_tween.queue_free()
	scrollbar_hide_tween = Tween.new()
	add_child(scrollbar_hide_tween)
	scrollbar_hide_tween.interpolate_property(get_v_scrollbar(), 'modulate', get_v_scrollbar().modulate, Color.transparent, scrollbar_fade_out_time)
	scrollbar_hide_tween.interpolate_property(get_h_scrollbar(), 'modulate', get_h_scrollbar().modulate, Color.transparent, scrollbar_fade_out_time)
	scrollbar_hide_tween.start()
...
func show_scrollbars() -> void:
	...
	scrollbar_hide_tween = Tween.new()
	add_child(scrollbar_hide_tween)
	scrollbar_hide_tween.interpolate_property(get_v_scrollbar(), 'modulate', get_v_scrollbar().modulate, Color.white, scrollbar_fade_in_time)
	scrollbar_hide_tween.interpolate_property(get_h_scrollbar(), 'modulate', get_h_scrollbar().modulate, Color.white, scrollbar_fade_in_time)
	scrollbar_hide_tween.start()
